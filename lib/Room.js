	...
	async _updateSpotlight(peerName)
	{
		...
		try
		{
			...
			for (const consumer of speaker.consumers)
			{
				...
				const filter = await this._getParticipantFilter(speaker.appData.displayName);

				// Media flow: rtpEndpoint -------> sipEndpoint
				await this._connectMediaElements(
					rtpEndpoint, filter, 'VIDEO'
				);

				// Media flow: rtpEndpoint -------> sipEndpoint
				await this._connectMediaElements(
					filter, sipEndpoint, 'VIDEO'
				);
	...
	async _handlePeer(peer)
	{
		...
		peer.on('newconsumer', (consumer) =>
		{
			...
			this._handleConsumer(consumer)
				.then(() =>
				{
					if (this._spotlights.currentSpotlight() === peer.name)
