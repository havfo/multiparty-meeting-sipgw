...
{
	constructor(roomId, srf, kurentoClient)
	{
		...
		this._spotlights = new Spotlights(this._room);
		...
		// Kurento GStreamFilter number of participants
	...
	async handleCall(req, res)
	{
		...
		try
		{
			...
			let localSdp = await this._processSipSdp(req.body, 'RtpEndpoint');
			...
			this._dialog.on('modify', async (modifyreq, modifyres) =>
			{
				...
				// This is the SDP from Kurento
				localSdp = await this._processSipSdp(modifyreq.body, 'RtpEndpoint');
	...
	{
		logger.debug('_getServerHistory()');
	...
	async _updateSpotlight(peerName)
	{
		...
		try
		{
			...
			if (!peer)
			{
				logger.error('_updateSpotlight() | peer not found');
			...
			for (const consumer of peer.consumers)
			{
				...
				const filter = await this._getParticipantsFilter(this._room.peers.length);

				// Media flow: rtpEndpoint -------> sipEndpoint
				await this._connectMediaElements(
					rtpEndpoint, filter, consumer.kind.toUpperCase()
				);

				// Media flow: rtpEndpoint -------> sipEndpoint
				await this._connectMediaElements(
					filter, sipEndpoint, consumer.kind.toUpperCase()
				);
	...
	async _getParticipantsFilter(number)
	{
		...
		return new Promise((resolve, reject) =>
		{
			pipeline.create('GStreamerFilter', {
				command: `textoverlay font-desc="Sans 24" text="${number}" valignment=top halignment=left shaded-background=true`
			}, (error, filter) =>
			{
				if (error)
				{
	...
	{
		logger.debug('_disconnectMediaElements()');
		...
		return new Promise((resolve, reject) =>
		{
			fromElement.disconnect(toElement, (error) =>
			{
				...
				logger.info('_disconnectMediaElements | successfully disconnected endpoints');
